...
describe("dashboard nav scope clarity", () => {
  ...
  test("superadmin sees platform and selected-school groupings without feature flags in primary nav", () => {
    ...
    expect(screen.getAllByText("Platform LLM").length).toBeGreaterThan(0);
  ...
  test("selected-school tools stay visibly gated until a school is chosen", () => {
    ...
    expect(screen.getAllByText("Choose School").length).toBeGreaterThan(0);
    expect(screen.getByRole("link", { name: "School Users" })).toHaveAttribute("aria-disabled", "true");
    expect(screen.getByRole("link", { name: "School Analytics" })).toHaveAttribute("aria-disabled", "true");
  });
